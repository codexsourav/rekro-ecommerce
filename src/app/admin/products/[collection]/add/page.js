...
function ProductAdd({ params }) {
    ...
    const SendAddRequest = () => {
        ...
        const requestData = {
            ...
            color: colors,
            desc: content,
            productcollection: params?.collection ?? "",
        ...
        Axios.request(axiosConfig)
            .then(function (response) {
                ...
                if (response.data.add === true) {
                    ...
                    // Clear other inputs as needed

                    // Optionally, you can reset the state variables if you have any
                    setContent("");
                    setSize([]);
                    setcolor([]);
                    setimages([]);
                    stock.current.value = "";
                    sortdesc.current.value = "";
                } else {
    ...
    return (
        <div>
            <AdminWrapper />
            <Container>
                <h1 className="mt-4 mb-5 text-center" >ADD New Product</h1>
                ...
                <Button variant="primary" size="lg" disabled={loading} className="mt-4 float-start mb-5" onClick={SendAddRequest}>{loading ? "Loading.." : "ADD NEW PRODUCT"}</Button>
            </Container>
        </div>
    )
}
